/**
 * Migration script: Import categories, subcategories, and keywords from CSV
 * 
 * Strategy:
 * 1. Create all new categories from CSV
 * 2. Map old subcategories (with requests/vendors) to new CSV structure
 * 3. Import all CSV subcategories and keywords
 * 4. Clean up orphans (Interior, Neumáticos, duplicate Partes Eléctricas)
 */
...
// ── Parse CSV ─────────────────────────────────────────────────────────────
interface CsvRow { category: string; subcategory: string; keywords: string[]; }
...
// ── Mapping: old subcategory → new (category, subcategory) ───────────────
// For subcategories that have requests/vendor data and need reassignment
const SUBCATEGORY_MAP: Record<string, { newCat: string; newSub: string }> = {
  ...
  // Interior → Accesorios/Carroceria
...
async function migrate() {
  ...
  // ── Step 1: Create all new categories from CSV ─────────────────────────
  ...
  // ── Step 2: Create all CSV subcategories ────────────────────────────────
  console.log(`\n=== Step 2: Creating subcategories from CSV ===`);
  const subcategoryMap: Record<string, string> = {}; // "cat|sub" → id
  let subsCreated = 0;
  let subsExisted = 0;

  for (const row of csvRows) {
    const catId = categoryMap[row.category];
    if (!catId) { console.error(`  Missing category for: ${row.category}`); continue; }
  ...
  // ── Step 3: Reassign old subcategories ──────────────────────────────────
  ...
  for (const oldSub of oldSubs) {
    ...
    if (!mapping) continue; // Not in the mapping, handled later
    ...
    if (targetSubId === oldSub.id) continue; // Same subcategory

    const hasData = oldSub._count.requests > 0 || oldSub._count.vendorPartSubcategories > 0;

    if (hasData) {
      // Move requests to target subcategory
      ...
      // Move vendor associations (delete old, create new if not exists)
    ...
    // Also update requests that reference the old category but not this subcategory
  ...
  // ── Step 4: Handle remaining requests pointing to old categories ───────
  ...
  // ── Step 5: Delete old keywords and import new ones ─────────────────────
  ...
  // ── Step 6: Clean up old subcategories with no data ─────────────────────
  ...
  // Determine which subcategories belong to CSV structure
  ...
  for (const sub of allOldSubs) {
    ...
    if (csvSubKeys.has(key)) continue; // Part of new structure

    // Check if it still has data
  ...
  // ── Step 7: Clean up old categories ─────────────────────────────────────
  ...
  for (const catName of toDelete) {
    ...
    // Check if still has subcategories
