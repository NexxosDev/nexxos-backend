...
let VendorService = VendorService_1 = class VendorService {
    ...
    constructor(prisma, clientPointsService) {
        this.prisma = prisma;
    ...
    async getVendorProfile(userId) {
        ...
        if (vendor.logoUrl) {
            try {
                logoUrl = await (0, s3_1.getFileUrl)(vendor.logoUrl, true);
            }
            catch {
                logoUrl = null;
            }
        ...
        if (vendor.facadeImageUrl) {
            try {
                facadeImageUrl = await (0, s3_1.getFileUrl)(vendor.facadeImageUrl, true);
            }
            catch {
                facadeImageUrl = null;
            }
    ...
    async updateVendor(userId, dto) {
        ...
        if (dto.logoPath !== undefined)
            data.logoUrl = dto.logoPath;
        if (dto.documentImagePath !== undefined)
            data.documentImageUrl = dto.documentImagePath;
        if (dto.personalDocPath !== undefined)
            data.personalDocUrl = dto.personalDocPath;
        if (dto.selfiePath !== undefined)
            data.selfieUrl = dto.selfiePath;
        if (dto.facadeImagePath !== undefined)
            data.facadeImageUrl = dto.facadeImagePath;
        ...
        if (dto.municipality !== undefined)
            data.municipality = dto.municipality;
        if (dto.parish !== undefined)
            data.parish = dto.parish;
        ...
        if (dto.referencePoint !== undefined)
            data.referencePoint = dto.referencePoint;
        ...
        if (updated.logoUrl) {
            try {
                logoUrl = await (0, s3_1.getFileUrl)(updated.logoUrl, true);
            }
            catch {
                logoUrl = null;
            }
        ...
        if (updated.facadeImageUrl) {
            try {
                facadeImageUrl = await (0, s3_1.getFileUrl)(updated.facadeImageUrl, true);
            }
            catch {
                facadeImageUrl = null;
            }
    ...
    async getDashboard(userId) {
        ...
        return {
            ...
            metrics: vendor.vendorMetrics
                ...
                : { totalRequestsReceived: 0, totalRequestsAnswered: 0, avgRating: null, totalRatings: 0 },
            recentRequests: await Promise.all(vendor.requestVendorMatches.map(async (m) => {
    ...
    async getResponseTimeMetrics(userId) {
        const vendor = await this.prisma.vendor.findUnique({ where: { userId } });
        if (!vendor)
            throw new common_1.NotFoundException('Vendor profile not found');
    ...
    async createQuickReply(userId, messageText) {
        const vendorId = await this.getVendorId(userId);
    ...
    async updateQuickReply(userId, replyId, messageText) {
        const vendorId = await this.getVendorId(userId);
    ...
    async deleteQuickReply(userId, replyId) {
        const vendorId = await this.getVendorId(userId);
        const reply = await this.prisma.quickReply.findFirst({
            where: { id: replyId, vendorId },
        });
    ...
    async reorderQuickReplies(userId, items) {
        const vendorId = await this.getVendorId(userId);
        await this.prisma.$transaction(items.map((item) => this.prisma.quickReply.updateMany({
